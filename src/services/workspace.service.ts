...
export class WorkspaceService {
  ...
  static async requireMembership(workspaceId: string, userId: string) {
  ...
  static async requireRole(
    ...
    const member = await WorkspaceService.requireMembership(
      workspaceId,
      userId,
    );
  ...
  static async inviteMember(
    ...
    await WorkspaceService.requireRole(workspaceId, invitedBy, [
      "owner",
      "admin",
    ]);
  ...
  static async acceptInvite(userId: string, token: string) {
    ...
    if (invite.acceptedAt)
      throw new AppError("This invite was already used", 400);
